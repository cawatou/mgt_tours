...
while ($arSection = $rsSections->Fetch()) {
	...
	while ($ob = $res->GetNextElement()) {
		...
		$arSelect = Array("ID", "NAME", "PROPERTY_TOURIDBX", "PROPERTY_hotelcode", "PROPERTY_LINK", "PROPERTY_hotelstars", "PROPERTY_nights", "PROPERTY_price", "PROPERTY_meal", "PROPERTY_operatorname", "PROPERTY_img", "PROPERTY_tourid");
...
<div class="row">
	<div class="col-md-12 col-xs-12 topturinfo">
		<h1 class="icon_tour"><?=$tours[0]['PROPERTY_COUNTRY_VALUE']?>, <p><?=$tours[0]['PROPERTY_CURORT_VALUE']?></p> <span>от <?=$tours[0]['PROPERTY_MIN_PRICE_VALUE']?> Р </span></h1>
				...
				<?if(isset($tours[$i]['ID'])):?>
					<div class="col-md-3 col-xs-3 ">
						<p class="fly_date" data-id="<?=$tours[$i]['ID']?>">
							...
							<span style="font-size: 14px;">на <?=$tours[$i]['PROPERTY_DAYCOUNT_VALUE']?> дней</span>
			...
			<div data-id="<?=$tour['ID']?>" class="tabwrap">
				<div class="hotel-carusel">
					<?foreach($tour['hotels'] as $hotel):?>
						<div class="item">
							<div class="hotel" style="background: url(<?='http://'.$hotel['PROPERTY_IMG_VALUE']?>) no-repeat center center; background-size: cover; box-shadow: 0 0 0 128px rgba(20, 10, 0, 0.43) inset;">
								<h4 onclick="window.open('<?if(empty($hotel['PROPERTY_LINK_VALUE'])){ echo ""; }else{?>/catalog/hotel/<?=$hotel['PROPERTY_HOTELCODE_VALUE']?><?}?>','_blank'); return false;"><?=substr($hotel['NAME'], 0, 17)?></h4>

								<div class="hotel_star"> 
									<?for($i=0; $i < $hotel['PROPERTY_HOTELSTARS_VALUE']; $i++):?>
...
<?
						...
						?> Р</span>
								...
								<!-- p><img src="/bitrix/templates/tour/images/paladdin/calendar.png" alt=""> <?=$hotel['PROPERTY_NIGHTS_VALUE']?> ночей<br --><?if(CSite::InGroup(array(1,10))){?><span style='color:white;'>Турагентство: <span style='color:white;'><?=$hotel['PROPERTY_OPERATORNAME_VALUE']?></span></span><?}?></p>
